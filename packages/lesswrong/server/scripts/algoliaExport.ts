...
import {
  ...
  algoliaIndexDocumentBatch,
  algoliaDeleteIds,
  subsetOfIdsAlgoliaShouldntIndex,
  algoliaGetAllDocuments,
...
async function algoliaExport(
  ...
  const timeBound =
    forumTypeSetting.get() === "EAForum" && !isProductionDBSetting.get()
...
async function algoliaCleanIndex(collectionName: AlgoliaIndexCollectionName) {
  ...
  // eslint-disable-next-line no-console
  console.log(
    `Deleting spurious documents from Algolia index ${getAlgoliaIndexName(collectionName)} for ${collectionName}`
  );
  ...
  // eslint-disable-next-line no-console
  console.log(
    `Deleting ${mongoIdsToDelete.length} mongo IDs (${algoliaIdsToDelete.length} algolia IDs) from Algolia...`
  );
