...
// to all posts.)
const imageUrlWhitelist = ["cloudinary.com", "res.cloudinary.com", "www.lesswrong.com", "www.alignmentforum.org"];

function urlNeedsMirroring(url: string) {
  const parsedUrl = new URL(url);
  if (imageUrlWhitelist.indexOf(parsedUrl.hostname) !== -1) return false;
  return true;
}
