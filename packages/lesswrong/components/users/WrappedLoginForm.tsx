...
const WrappedLoginFormDefault = ({ startingState = "login", classes }: WrappedLoginFormProps) => {
  ...
  const [subscribeToCurated, setSubscribeToCurated] = useState<boolean>(
    !["EAForum", "AlignmentForum"].includes(forumTypeSetting.get())
  );
  ...
  const submitFunction = async (e) => {
    ...
    const variables =
      currentAction === "signup"
        ? { email, username, password, reCaptchaToken, abTestKey: signupAbTestKey, subscribeToCurated }
        : currentAction === "login"
        ? { username, password }
        : currentAction === "pwReset"
        ? { email }
        : {};
  ...
  return (
    <React.Fragment>
      ...
      <form className={classes.root} onSubmit={submitFunction}>
        {["signup", "pwReset"].includes(currentAction) && (
          <input
            value={email}
            type="text"
            name="email"
        ...
        <div className={classes.options}>
          {currentAction !== "login" && (
            <span className={classes.toggle} onClick={() => setCurrentAction("login")}>
              {" "}
              Log In{" "}
            </span>
          )}
          {currentAction === "login" && (
            <span className={classes.toggle} onClick={() => setCurrentAction("signup")}>
              {" "}
              Sign Up{" "}
            </span>
          )}
          ...
          {currentAction !== "pwReset" && (
            <span className={classes.toggle} onClick={() => setCurrentAction("pwReset")}>
              {" "}
              Reset Password{" "}
            </span>
          )}
  ...
  return (
    <div className={classes.root}>
      <div className={classnames(classes.oAuthBlock, "ea-forum")}>
        <a className={classes.oAuthLink} href={`/auth/auth0?returnTo=${pathname}`}>
          Login
        </a>
        <a className={classes.oAuthLink} href={`/auth/auth0?screen_hint=signup&returnTo=${pathname}`}>
          Sign Up
        </a>
      </div>
    </div>
